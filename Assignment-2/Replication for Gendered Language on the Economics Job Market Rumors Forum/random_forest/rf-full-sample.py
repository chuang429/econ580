...
def is_noise_word(word):
    if word in ENGLISH_STOP_WORDS:
        return True
    if re.fullmatch(r"[0-9]+", str(word)):   # pure numbers
        return True
    if re.fullmatch(r"\W+", str(word)):      # punctuation
        return True
    if len(str(word)) < 2:                   # single letters
        return True
    return False

noise_vocab = vocab10K[vocab10K['word'].apply(is_noise_word)]
i_exclude_noise = noise_vocab['index'] - 1

# combine original exclusions with noise exclusions
i_exclude = list(set(i_exclude).union(set(i_exclude_noise)))
# ===========================================================
